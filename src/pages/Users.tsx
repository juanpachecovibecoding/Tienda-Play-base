...
import { Users as UsersIcon, Shield, Edit2, Check, X } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col gap-6 h-full min-h-0">
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl flex gap-3 items-start">
        ...
        <p className="text-sm text-indigo-800 font-medium leading-relaxed">
          Los nuevos usuarios se agregan automáticamente como "vendedor" al iniciar sesión por primera vez con su cuenta de Google. Puedes cambiar sus permisos aquí.
        </p>
      </div>
                  ...
                  <td className="px-6 py-4 text-right">
                    {editingId === user.id ? (
                      <div className="flex justify-end space-x-2">
                        <button onClick={() => saveRole(user.id)} className="p-1 text-emerald-600 hover:bg-emerald-50 rounded"><Check size={16} /></button>
                      ...
                      <button 
                        ...
                        disabled={user.email === 'juanpacheco@playcode.com.ar' || user.email === 'juanpacheco.vibecoding@gmail.com'}
                      >
                        <Edit2 size={16} />
                      </button>
                    )}
